...
/**
 * 核心 Core Function: Updates the persistent ledger
 */
function updatePaymentLedger(startDate, endDate) {
  ...
  // Create Map of Existing Rows: Key = "SessionID_TeacherName" (Unique for Event)
  // Why Name? Because one session ID can generate 2 rows (one for actual teacher, one for original)
  const ledgerMap = new Map();
  const ledgerData = sheetLedger.getDataRange().getValues();
  for (let i = 1; i < ledgerData.length; i++) {
      // Key: SessionID + TeacherName (To distinguish Cover vs Missed for same session ID)
  ...
  if (sheetUnavail) {
    ...
    for (let i = 1; i < uaData.length; i++) {
      ...
      unavailMap.set(`${uaData[i][1]}_${uaData[i][2]}_${normalizeTime(uaData[i][4])}`, uaData[i][6]);
    }
  }

  // 4. Process Logic & Prepare Writes
  const sessionData = sheetFact.getDataRange().getValues();
  const rowsToUpdate = []; // { row: idx, values: [] }
  const rowsToInsert = []; // []
  ...
  for (let i = 1; i < sessionData.length; i++) {
      ...
      let startTime = normalizeTime(row[2]);
      let endTime = normalizeTime(row[3]);
      ...
      // --- LOGIC A: Actual Teacher (Teaching/Cover) ---
      ...
      // --- LOGIC B: Original Teacher (Cancelled/Missed) ---
      if (originalId && userMap.has(originalId)) {
          ...
          if (status.startsWith("Cancelled")) {
              ...
              if (userReason) {
                  ...
                  payStatus = "⚠️ Review";
                  unit = 0; // Force no pay if teacher has reason
              } else {
                  if (isPayable) {
...
// Helper to decide Update vs Insert
...
function exportFinanceFiles() {
  ...
  let dateStr = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "yyyy-MM-dd");
